...
class SalaRayosX extends SalaMedica {
...
class Consultorio extends SalaMedica {
...
// const s = new SalaMedica(); // Error: Cannot create an instance of an abstract class.

const salaRayosX = new SalaRayosX(5);
const consultorio = new Consultorio(4, 6);

console.log(salaRayosX.describir()); // Área: 78.54 | Perímetro: 31.42
console.log(consultorio.describir());    // Área: 24.00 | Perímetro: 20.00
...
class TarjetaCredito extends MetodoPago {
  ...
  override procesar(monto: number): string {
    ...
    return `Tarjeta ****${this.ultimos4}: $${monto} de consulta aprobado. Saldo restante: $${this.saldoDisponible}`;
  }
}
...
class TransferenciaBancaria extends MetodoPago {
  ...
  override procesar(monto: number): string {
    return `Transferencia de $${monto} para consulta de ${this.titular} a CLABE ${this.clabe.slice(-4).padStart(18, "*")}.`;
  }
}
...
// Tarjeta ****4321: $200 de consulta aprobado. Saldo restante: $300

console.log(transferencia.resumen(1000));
// Transferencia de $1000 para consulta de Luis a CLABE **************5678.
